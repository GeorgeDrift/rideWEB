...
const StatCard = ({
    ...
    <div className={`p-6 rounded-xl border transition-all duration-300 group hover:shadow-lg ${active
...
export const TotalRidesPage: React.FC<TotalRidesPageProps> = ({ onBack }) => {
    ...
    const getTotalBreakdown = () => {
        const data = getData() as Array<{ share: number, hire: number }>;
        const share = data.reduce((acc, curr) => acc + (curr.share || 0), 0);
        const hire = data.reduce((acc, curr) => acc + (curr.hire || 0), 0);
        return { share, hire };
    };

    const { share: totalShare, hire: totalHire } = getTotalBreakdown();
    const rideShareValue = totalShare.toLocaleString();
    const forHireValue = totalHire.toLocaleString();

    // Calculate mock breakdown values based on total (if not available in API, use estimation or additional data)
    // Assuming ridesData contains breakdown or we use estimation from total for now as per previous logic
    const completedValue = ridesData.breakdown?.completed?.toLocaleString() || Math.floor((totalRides ?? 0) * 0.92).toLocaleString();
    const cancelledValue = ridesData.breakdown?.cancelled?.toLocaleString() || Math.floor((totalRides ?? 0) * 0.08).toLocaleString();
                ...
                <div className="bg-white dark:bg-dark-800 border border-gray-300 dark:border-dark-600 p-3 rounded-lg shadow-xl">
                    ...
                    {payload[1] && (
                        <p className="text-blue-500 text-xs font-medium mt-1">
                            {payload[1].name}: {(payload[1].value ?? 0).toLocaleString()}
                        </p>
                    )}
                </div>
            );
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={monthlyData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151', color: '#f3f4f6' }}
                                />
                                <Area type="monotone" dataKey="share" stroke="#FACC15" fillOpacity={1} fill="url(#colorShare)" name="Ride Share" />
                                <Area type="monotone" dataKey="hire" stroke="#3b82f6" fillOpacity={1} fill="url(#colorHire)" name="For Hire" />
